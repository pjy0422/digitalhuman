...
@agent_registry.register("evaluator_final")
class EvaluatorFinalAgent(BaseAgent):
    max_history: int = 5
    ...
    async def astep(
        ...
        print("evaluator final solution--", solution)
        ...
        # print(f"\033[95m{self.name}\033[0m")
        # print(self.max_history, prepend_prompt, history, append_prompt)
        # print(f"\033[95mEND{self.name}\033[0m")
        # for i in range(self.max_retry):
        #     try:
        print('eval history: ', history)
        if len(history) > 0 and history[-1] == {'role': 'user', 'content': '\n\nDiscussion history are as follows:\n[](Temporarily Empty)\n\n'}:
            history = history[:-1]
        print('eval history: ', history)
