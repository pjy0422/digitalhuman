...
class CriticAgent(BaseAgent):
    ...
    async def astep(
        ...
        **kwargs,
        ...
        # prepend_prompt = f"You are [{self.name}]. " + prepend_prompt
    ...
    def gameover(self) -> None:
        """After being killed from the game"""
        message = CriticMessage(
            content="SYSTEM: "+ self.name + "has lost the discussion and will be removed from the platform.",
            sender=self.name,
            sender_agent=self,
            is_agree=False,
        )
...
#     ) -> CriticMessage:
#         """Asynchronous version of step"""
#         if self.agent_score!=None and self.agent_score < 0:
