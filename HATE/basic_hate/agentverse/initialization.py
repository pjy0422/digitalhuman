...
def prepare_task_config(task, tasks_dir, postfix=''):
    """Read the yaml config of the given task in `tasks` directory."""
    all_task_dir = tasks_dir
    task_path = os.path.join(all_task_dir, task)
    # print(all_task_dir, task)
    print('all_task_dir: ', all_task_dir)
    print('task: ', task)
    print('task_path: ', task_path)
    print('task_path exist? ', os.path.exists(task_path), task_path)
    config_path = os.path.join(task_path, f"config_{postfix}.yaml")
    print(config_path)
